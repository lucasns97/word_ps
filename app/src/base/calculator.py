...
class Calculator:
    ...
    def _pairwise_sim(self, trg: str, hyp: str, max_steps: int=None) -> tuple:
    ...
    def _exc_pairwise_sim(self, trg: str, hyp: str, bidirectional: bool=False, weights: list=[2,1]) -> tuple:
        ...
        if bidirectional:
            ...
            sc1, wg = self._pairwise_sim(trg, hyp)
            sc2, _  = self._pairwise_sim(trg[::-1], hyp[::-1])
            ...
            sc, wg = self._pairwise_sim(trg, hyp)

        return sc, wg

    def weighted_similarity(self, trg: str, hyp: str, bidirectional: bool=False, split_method: str="split") -> float:
        ...
        for maximizer_text in maximizer_grams:
            ...
            for min_w, max_w in zip(minimizer, maximizer):
                ...
                sc, wg = self._exc_pairwise_sim(min_w, max_w, bidirectional)
                score  += sc*wg
                weight += wg
            
        return score/weight

    def ngrams_weighted_similarity(self, trg: str, hyp: str, bidirectional: bool=False, split_method: str="split", ngrams: int = 3) -> float:
        ...
        for i in range(1, ngrams+1):
            ...
            for trg_ngram, hyp_ngram in zip(trg_ngrams, hyp_ngrams):
                ...
                # Accumulate score
                ngram_score = ngram_score + self.weighted_similarity(trg_ngram, hyp_ngram, bidirectional, split_method)
                loop_count += 1
